...
export class RouterAIClient implements ChatClient {
  ...
  constructor(
    ...
    if (![PRIMARY_MODEL, FALLBACK_MODEL].includes(defaultModel)) throw new Error('routerai_model_not_allowed');
  }
  ...
  fallback(): RouterAIClient | undefined {
    return this.defaultModel === PRIMARY_MODEL
      ? new RouterAIClient(this.env, FALLBACK_MODEL, this.transport, this.observe) : undefined;
  }
  async call(options: DeepSeekRequestOptions): Promise<string> {
    ...
    const secondary = this.defaultModel === FALLBACK_MODEL;
