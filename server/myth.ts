...
export const PERSONAL_MYTH_WRITER_VERSION = "personal-myth-v1.5-emergent-possibility";
...
const EMERGENT_POSSIBILITY_CUE = /(?:стало|становится|оказалось|оказывается|появил(?:ась|ось|ся)|появляется|открыл(?:ась|ось|ся)|открывается|можно|возможност|доступн|позволяет|получается)/iu;
const HIDDEN_ESSENCE_CLAIM = /(?:скрыт\w*\s+(?:сила|талант|ресурс)|в\s+тебе\s+(?:есть|всегда)|ты\s+(?:обладаешь|умеешь)|тво[яё]\s+способност)/iu;
const INVENTED_CONFLICT_WHEN_NONE_GIVEN = [
  ...
  /не\s+знал[аи]?,?\s+что\s+[^.!?\n]{0,80}(?:напряж|зажат)/iu,
  /(?:плечи|тело|мышцы)\s+[^.!?\n]{0,50}(?:отпуска|расслаб)/iu,
  /не\s+помнишь,?\s+когда\s+в\s+последний\s+раз/iu,
  /раньше\s+ты\s+бы/iu,
  /(?:твоя|твоей|твою)\s+привычк/iu,
  /обычно\s+ты\s+(?:делаешь|делал|поправляешь|убираешь|избегаешь)/iu,
  /ты\s+не\s+помнишь,?\s+[^.!?\n]{0,100}/iu,
  /дольше,?\s+чем\s+обычно/iu,
  /между\s+привычн\w*\s+положен/iu,
];
...
export function buildPersonalMythPromptV11(
  ...
  if (EXPLICIT_NO_CONFLICT.test(request.answers.q1)) {
    adaptiveFidelityRules.push(
      "Пользователь прямо сообщил, что острого конфликта или проблемы нет. Не создавай скрытое затруднение, страх, нерешительность, неспособность начать или драму ради сюжета. Не приписывай телу скрытое напряжение и последующее облегчение; не выдумывай прошлое поведение, привычку, норму «обычно», конструкцию «раньше ты бы…» или «ты не помнишь…». Не сравнивай сцену с неописанным прошлым человека. Развивай спокойное любопытство и наблюдение самого настоящего момента.",
    );
  }
  ...
  const adaptiveFidelityBlock = adaptiveFidelityRules.length
    ...
    : "";
  return `Ты — зрелый русскоязычный писатель. Создай «Личный миф»: образную литературную историю для саморефлексии, которая помогает человеку увидеть нынешнее состояние и небольшую ещё не обязательную возможность действия через метафору. Это образное полотно, а не совет или психологический анализ.
...
4. Q4 КАК НАПРАВЛЕНИЕ ВОЗМОЖНОГО РАСКРЫТИЯ:
   ...
   - Пусть развязка открывает выбор, но не объявляет человека исцелённым или изменившимся. Заверши историю наблюдаемым образом, жестом, фактурой, звуком, расстоянием или открытым выбором. Не добавляй после него авторский вывод о том, что теперь произошло с человеком, и не возвращай слова q1–q4 как окончательный вердикт.
   - Ответ q4 (искомое качество) задаёт направление возможного раскрытия, но НЕ является правильным финалом, который история обязана выполнить. Не приводи человека к заявленному качеству автоматически и не изображай исполнение желания. Покажи только небольшой жест, изменение отношения или новую возможность, совместимую с этим направлением. Не обесценивай и не отрицай q4. Перерабатывай образы метафорически, не копируя фразы пользователя механически.
   - В newView предложи вопрос или возможный ракурс на механизм самой сцены; оставь читателю право решить, применим ли он к ситуации. Не объявляй причиной поведения пользователя и не пересказывай мораль истории. Новый ракурс должен соединять минимум два разных ответа.
...
4а. ЧТО СТАЛО ВОЗМОЖНЫМ — ЧЕРЕЗ ДЕЙСТВИЕ, А НЕ ОБЪЯВЛЕНИЕ:
   ...
   - Дай этому способу сначала проявиться в малом действии внутри сцены. Затем пусть его можно будет попробовать рядом с предметом из q2 или вопросом из q1. Возможность узнаётся по наблюдаемому изменению следующего жеста, расстояния или способа обращения с предметом, а не по похвале рассказчика.
   ...
   - Техническое поле hiddenResource имеет публичный смысл «Что стало возможным». Назови в нём конкретную возможность, уже показанную действием внутри сцены. Используй язык возможности («стало возможно», «можно», «открылась возможность»), а не скрытой сущности. В newView сохраняй открытость переноса в жизнь. Не расшифровывай историю как единственно верную мораль.
...
5. ЛИТЕРАТУРНОЕ КАЧЕСТВО:
   ...
   - Story, mirror, meaning, one_step и journal_question должны дополнять друг друга. Не пересказывай одну и ту же мораль пять раз. После сильного финального образа оставь паузу: не объясняй читателю значение последней сцены.
   - Внутреннее поле hiddenResource означает «Что стало возможным». Слово «ресурс» допустимо только как техническое имя JSON-поля, но не в тексте для читателя.
...
Верни только валидный JSON без markdown:
{
  ...
  "story_result": {
    ...
    "mirror": {
      ...
      "hiddenResource": "что стало возможным В СЦЕНЕ после конкретного жеста; язык возможности, не скрытой силы и не устойчивой способности человека",
      "newView": "вопрос или возможный ракурс на новый ход образа, соединяющий два ответа; без готового вывода, предыстории, числа попыток или пути к успеху"
    },
...
export function formatBlockerForRepair(blocker: string): string {
  switch (blocker) {
    ...
    case "emergent_possibility_contract":
      return "Поле hiddenResource имеет смысл «Что стало возможным». Назови конкретную возможность, уже показанную действием в story. Используй язык возможности; не объявляй скрытую силу, талант или устойчивую способность человека.";
...
7. Покажи, что стало возможным после малого обратимого действия. Эта возможность должна быть видна в story и совместима с направлением q4, но не обязана исполнять его. Не объявляй скрытый талант, исцеление или завершённое внутреннее изменение.
...
Верни ТОЛЬКО валидный JSON строго следующей структуры:
{
  ...
  "story_result": {
    ...
    "mirror": {
      ...
      "hiddenResource": "что стало возможным в сцене после конкретного жеста (на ты; не скрытая сила)",
...
export function validatePersonalMythResult(result: PersonalMythResult, request?: PersonalMythRequest): PersonalMythQualityReport {
  ...
  if (
    request &&
    (!EMERGENT_POSSIBILITY_CUE.test(result.mirror.hiddenResource) || HIDDEN_ESSENCE_CLAIM.test(result.mirror.hiddenResource))
  ) {
    blockers.push("emergent_possibility_contract");
  }
