...
const evidenceDir = path.join(repoRoot, "docs", "evidence", "owner-deployment-acceptance");
...
async function runProductionAcceptance() {
  console.log("====================================================================");
  console.log("=== ISSUE #23: OWNER-ONLY REAL-DOMAIN DEPLOYMENT ACCEPTANCE ===");
  ...
  // -------------------------------------------------------------
  // 1. Health & Readiness Verification on Production Domain
  // -------------------------------------------------------------
  console.log("[QA-1] Checking /health and /health/ready on live production domain...");
  ...
  try {
    ...
    // =============================================================
    // SECTION 2: Real-Domain Phone-First Journey (390x844)
    // =============================================================
    console.log("\n=============================================================");
    console.log("=== REAL-DOMAIN JOURNEY (390x844) ===");
    console.log("=============================================================");
    ...
    await pagePhone.goto(targetUrl);
    await pagePhone.waitForLoadState("networkidle");

    // Screenshot 1: Threshold
    console.log("[Screenshot 1/9] Capturing production threshold (01-prod-threshold-390x844.png)...");
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "01-prod-threshold-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });

    // 1. Personal Myth Lens
    console.log("[Production Journey] Entering Personal Myth...");
    ...
    const mythData = await completeMythStepper(pagePhone, {
      q1: "поиск равновесия между незыблемой структурой и стихией",
      q2: "каменный маяк на скале посреди вечернего моря",
      q3: "тишина перед приближающейся грозой",
      q4: "уверенность, глубина и спокойное присутствие",
    });
    ...
    if (
      ...
      throw new Error(`Production Myth response provenance failure: ${JSON.stringify(mythData)}`);
    }
    ...
    console.log("  ✓ Production Live Myth Result:", recordedProvenance.myth.title);
    console.log("  ✓ Production Myth Provenance Asserted:", { provider: mythData.provider, model: mythData.model });

    // Screenshot 2: Myth Result
    console.log("[Screenshot 2/9] Capturing production Myth (02-prod-myth-390x844.png)...");
    ...
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "02-prod-myth-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });

    // 2. Digital Code Lens (15.08.1990)
    console.log("[Production Journey] Navigating to Digital Code...");
    ...
    console.log("[Screenshot 3/9] Capturing production Code (03-prod-code-390x844.png)...");
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "03-prod-code-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });

    // 3. Meeting of Mirrors
    console.log("[Production Journey] Navigating to Meeting of Mirrors and requesting LIVE production synthesis...");
    ...
    console.log("  [Meeting] Waiting for LIVE production /api/lab/meeting/generate response...");
    ...
    if (
      ...
      throw new Error(`Production Meeting response provenance failure: ${JSON.stringify(synthJson)}`);
    }
    ...
    console.log("  ✓ Production Live Meeting Result:", recordedProvenance.meeting.summary);
    console.log("  ✓ Production Meeting Provenance Asserted:", { provider: synthJson.provider, model: synthJson.model });
    ...
    console.log("[Screenshot 4/9] Capturing production Meeting (04-prod-meeting-390x844.png)...");
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "04-prod-meeting-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });

    // 4. Web Albert Dialogue (Live Production)
    console.log("[Production Journey] Opening Web Albert and sending live question...");
    ...
    if (
      ...
      throw new Error(`Production Albert response provenance failure: ${JSON.stringify(albertJson)}`);
    }
    ...
    console.log("  ✓ Production Live Albert Reply:", recordedProvenance.albert.messagePreview);
    console.log("  ✓ Production Albert Provenance & Contract Asserted:", { provider: albertJson.provider, model: albertJson.model, words: albertContract.wordCount });

    await pagePhone.waitForTimeout(1000);

    // Screenshot 5: Albert Reply
    console.log("[Screenshot 5/9] Capturing production Albert reply (05-prod-albert-390x844.png)...");
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "05-prod-albert-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });
    ...
    // -------------------------------------------------------------
    // 5. My Mirror V0 Local Persistence Flow on Real Domain
    // -------------------------------------------------------------
    console.log("\n[Production Persistence] Testing My Mirror V0 persistence on live domain...");

    const noteField = pagePhone.locator('textarea[placeholder*="Запишите мысли"]').first();
    await noteField.scrollIntoViewIfNeeded();
    await noteField.fill("Продакшн-заметка о встрече зеркал (Real Domain Acceptance)");
    ...
    console.log("[Screenshot 6/9] Capturing production My Mirror saved (06-prod-my-mirror-saved-390x844.png)...");
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "06-prod-my-mirror-saved-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });

    // Real Page Reload
    console.log("  [Real Reload] Performing browser reload on live domain...");
    ...
    console.log("[Screenshot 7/9] Capturing production reload saved entry (07-prod-reload-saved-390x844.png)...");
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "07-prod-reload-saved-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });

    // Explicit Restore with 0 Provider Calls Assertion
    console.log("  [Explicit Restore] Restoring saved mirror and asserting 0 provider calls on live domain...");
    ...
    console.log(`  ✓ Provider calls on real domain restore: ${restoreProviderCalls.length}`);
    if (restoreProviderCalls.length > 0) {
      throw new Error("Provider calls detected on real domain restore: " + restoreProviderCalls.join(", "));
    }

    // Screenshot 8: Restored Meeting
    console.log("[Screenshot 8/9] Capturing production restored Meeting (08-prod-restored-meeting-390x844.png)...");
    const restoredTitle = pagePhone.locator('text=Итог ·').first();
    await restoredTitle.scrollIntoViewIfNeeded();
    await pagePhone.waitForTimeout(400);
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "08-prod-restored-meeting-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });

    // -------------------------------------------------------------
    // 6. Restored Albert Request Context & Live Reply Check on Real Domain
    // -------------------------------------------------------------
    console.log("\n[Production Restored Albert] Opening Web Albert from restored Meeting and inspecting request payload context...");
    ...
    if (!capturedAlbertBody || !capturedAlbertBody.context) {
      throw new Error("Failed to capture outgoing context in /api/albert/dialogue request on production domain!");
    }

    const reqCtx = capturedAlbertBody.context;
    console.log("  [Production Restored Context Audit]:", {
    ...
    if (!reqCtx.meetingSummary || reqCtx.meetingSummary.length < 20) {
      throw new Error("Production restored Albert context missing meetingSummary!");
    }
    if (!reqCtx.codeAnchors || (!reqCtx.codeAnchors.numbers && !reqCtx.codeAnchors.keyInsight)) {
      throw new Error("Production restored Albert context missing codeAnchors!");
    }
    if (!reqCtx.mythAnchors || (!reqCtx.mythAnchors.title && !reqCtx.mythAnchors.mainImage)) {
      throw new Error("Production restored Albert context missing mythAnchors!");
    }
    ...
    if (
      ...
      throw new Error(`Production restored Albert failed provenance contract: ${JSON.stringify(albertRestoredRes)}`);
    }
    ...
    console.log("  ✓ Production restored Albert reply validated:", recordedProvenance.restoredAlbert.messagePreview);

    // Screenshot 9: Restored Albert Reply
    console.log("[Screenshot 9/9] Capturing production restored Albert reply (09-prod-restored-albert-390x844.png)...");
    await pagePhone.screenshot({
      path: path.join(evidenceDir, "09-prod-restored-albert-390x844.png"),
      clip: { x: 0, y: 0, width: 390, height: 844 },
    });
    ...
    // =============================================================
    // SECTION 3: Desktop Catastrophic Smoke (1440x900)
    // =============================================================
    console.log("\n=============================================================");
    console.log("=== DESKTOP SMOKE (1440x900) ===");
    console.log("=============================================================");
    ...
    console.log("[Desktop Screenshot 1/2] Capturing production desktop threshold (10-prod-desktop-threshold-1440x900.png)...");
    await pageDesk.screenshot({
      path: path.join(evidenceDir, "10-prod-desktop-threshold-1440x900.png"),
    ...
    console.log("[Desktop Screenshot 2/2] Capturing production desktop completed Meeting (11-prod-desktop-meeting-1440x900.png)...");
    ...
    await pageDesk.screenshot({
      path: path.join(evidenceDir, "11-prod-desktop-meeting-1440x900.png"),
      clip: { x: 0, y: 0, width: 1440, height: 900 },
    });
    ...
    try {
      prodLogs = execSync(
        `ssh -o BatchMode=yes -i ~/.ssh/id_ed25519_yc_dcs root@217.12.37.223 "journalctl -u zerkalo --since '${testStartTime}' --no-pager"`
      ).toString();
      console.log("Production logs during test window:\n", prodLogs);
    } catch (e) {
    ...
    fs.writeFileSync(
      path.join(evidenceDir, "production_provenance_summary.json"),
      JSON.stringify(
        {
          testStartTime,
          targetUrl,
    ...
    console.log("\n====================================================================");
    console.log("=== PRODUCTION ACCEPTANCE AUDIT COMPLETE ===");
    console.log("====================================================================");
    ...
    if (unexpectedErrors.length > 0) {
      throw new Error(`Unexpected product errors detected during production QA run: ${unexpectedErrors.join("; ")}`);
    }

    console.log("\n====================================================================");
    console.log("=== ALL PRODUCTION ACCEPTANCE CHECKS PASSED SUCCESSFULLY ===");
    console.log("====================================================================\n");
